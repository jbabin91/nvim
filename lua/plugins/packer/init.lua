require('packer').startup(function(use)
  ...
  use {
    'TimUntersberger/neogit', -- Manage git within Vim
    cmd = 'Neogit',
    config = function() require('plugins.neogit') end
  }
  use {
    'numToStr/comment.nvim', -- Filetype-aware automatic commenting (gcc)
    keys = 'gc',
    config = function() require('Comment').setup() end
  }
  use {
    'machakann/vim-sandwich', -- Quickly surround text objects with parentheses, etc.
    keys = 's'
  }
  use {
    'RRethy/vim-illuminate', -- highlight matching parentheses
    ft = { 'python', 'sh', 'bash', 'tex', 'lua', 'css', 'html', 'cpp' },
    config = 'vim.g.Illuminate_delay = 70'
  }
  use {
    'neovim/nvim-lspconfig', -- Language server linting and fixing
    ft = { 'python', 'sh', 'bash', 'tex', 'lua', 'css', 'html', 'cpp' },
  ...
  use {
    'folke/zen-mode.nvim', -- Distraction-free mode (<leader>g)
    cmd = 'ZenMode',
  ...
  use {
    'simnalamburt/vim-mundo', -- Graphical undo
    cmd = 'MundoToggle'
  }
  use {
