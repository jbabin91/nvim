...
local require_plugin = function(p)
  if JB.plugins[p] ~= nil and not JB.plugins[p].enabled then return nil end
...
local add_which_key = function(starter_key, definitions)
  JB.plugins.which_key.user_defined[starter_key] = definitions or {}
end
...
JB.utils = {
    ...
    copy = function(t1, t2)
      for k, v in pairs(t2) do
        if (type(v) == "table") and (type(t1[k] or false) == "table") then
          JB.utils.tables.copy(t1[k], t2[k])
        else
          t1[k] = v
        end
      end
  ...
  plugins = {
    ...
    add_user = function(p)
      table.insert(JB.plugins.user, p)
    end,
    -- Allows us to require packages in core-user-config
    -- without throwing an exception if the package doesn't exist
    -- Optionally you can run this like some/package to add it
    -- to the JB.packages.user table for installation.
    --
    ...
    require_if_installed = function(mod_name, pkg, conf)
      ...
      elseif pkg_is_git then
        table.insert(JB.plugins.user, {packer_ref, conf})
        return nil
      end
    end,
  },
